...
contract GhoToken is ERC20, Ownable, IGhoToken {
  ...
  function mint(address account, uint256 amount) external override {
    uint256 bucketCapacity = _facilitators[msg.sender].capacity;
    require(bucketCapacity > 0, 'INVALID_FACILITATOR');

    uint256 currentBucketLevel = _facilitators[msg.sender].level;
    uint256 newBucketLevel = currentBucketLevel + amount;
    require(bucketCapacity >= newBucketLevel, 'FACILITATOR_BUCKET_CAPACITY_EXCEEDED');
    _facilitators[msg.sender].level = uint128(newBucketLevel);
    ...
    onlyOwner
  {
    ...
    uint256 oldCapacity = _facilitators[facilitator].capacity;
    _facilitators[facilitator].capacity = newCapacity;
  ...
  function getFacilitatorBucket(address facilitator) external view returns (uint256, uint256) {
    return (_facilitators[facilitator].capacity, _facilitators[facilitator].level);
  }
    ...
    internal
  {
    ...
    require(facilitatorConfig.level == 0, 'INVALID_BUCKET_CONFIGURATION');

    facilitator.label = facilitatorConfig.label;
    facilitator.capacity = facilitatorConfig.capacity;
  ...
  function _removeFacilitator(address facilitatorAddress) internal {
    ...
    require(_facilitators[facilitatorAddress].level == 0, 'FACILITATOR_BUCKET_LEVEL_NOT_ZERO');
