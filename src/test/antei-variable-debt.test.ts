...
makeSuite('Antei VariableDebtToken End-To-End', (testEnv: TestEnv) => {
  ...
  it('Get Discount Rate Strategy', async function () {
    ...
    const discountRateStrategyAddress = await variableDebtToken.getDiscountRateStrategy();
    expect(discountRateStrategyAddress).to.be.equal(discountRateStrategy.address);
  });
});
