...
describe('Gho AToken Unit Test', () => {
  ...
  before(async () => {
    ...
    const ghoAToken_factory = await ethers.getContractFactory('GhoAToken');
    tempAToken = await ghoAToken_factory.deploy(
  ...
  it('Get Treasury', async function () {
    const ghoTreasury = await tempAToken.getTreasury();
